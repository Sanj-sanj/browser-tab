...
const Selections = ({ Component, label, state, _onClick, children }) => {
  ...
  return (
    <div className="flex flex-col w-100 justify-between cursor-default">
      <button
        className={`w-full relative px-7 py-1 flex items-center focus:outline-none cursor-default active:bg-yellow-500 focus:bg-pop-700  
        ${!toggle ? "hover:bg-pop-700" : ""}
      ...
      >
        {children.map((child) => (
          ...
          >
            <button
              className={`w-full pl-14 py-1 text-left cursor-default focus:outline-none bg-pop-900 focus:bg-pop-700 active:bg-yellow-500 hover:bg-pop-700 text-sm flex`}
              onClick={() => _onClick()}
